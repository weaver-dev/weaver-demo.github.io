...
| Software integrity verification on ePHI-processing systems | 164.312(c)(1) Required | **Private Nix Cache + Approved Packages (Decision WVR-147).** Attic-based substituter cache with signed derivations — software installed on ePHI-processing hosts has verified integrity and approved origin. Add-only signing-key rotation; tamper-evident approval audit log. Shed Builder (Decision WVR-149) with two-person rule at Team+ for binary drops prevents unauthorized software introduction that could alter or destroy ePHI processing integrity. | Planned (v2.3.0) | Weaver Team+ |
