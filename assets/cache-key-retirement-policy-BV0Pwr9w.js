...
<!-- weaver-free: never-publish — Dev-only — Docs: internal directories. Excluded from the Free mirror by .github/sync-exclude.yml (\`docs/operations/\`). -->
...
Weaver's Private Nix Cache uses an **add-only signing-key trust list** (Decision WVR-147). Here's what that means operationally:
...
**Why this design (Decision WVR-147 investigation, 2026-04-14):**
...
**Retired private keys are physically deleted from sops-nix at retirement.** This is intentional per Decision WVR-147 — eliminates the "sops recipient separation" concern (there's nothing to compromise in the sops file for a retired key).
...
| Date | Change | Author |
|---|---|---|
| 2026-04-15 | Initial draft — policy recommendations drawn from Decision WVR-147, NIST SP 800-57, and framework surveys | Claude (session 2026-04-14/15) |
