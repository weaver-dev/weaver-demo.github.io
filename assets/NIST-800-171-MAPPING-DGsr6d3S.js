...
| 3.4.8 | Apply deny-by-exception (blacklist) policy to prevent use of unauthorized software or deny-all, permit-by-exception (whitelist) policy to allow the execution of authorized software | **Private Nix Cache + Approved Packages (Decision WVR-147).** Attic-based substituter cache with manually-curated allowlist — no software can execute on approved hosts unless the derivation hash is in the approval table. Add-only signing-key rotation + audit log of every approval/revocation. Shed Builder (Decision WVR-149) provides custom software ingestion with two-person rule at Team+ for binary drops (separation-of-duties per 3.1.4). | Planned (v2.3.0) | Weaver Team+ |
