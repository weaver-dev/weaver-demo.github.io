...
**Unstable / \`nixpkgs-unstable\`:** Not tested or supported. NixOS stable provides reproducible builds — the same flake input always produces the same system. Unstable updates packages daily, which breaks this guarantee: the Node.js version, system libraries, or service defaults can change between any two evaluations. This directly impacts compliance posture — reproducible builds are a prerequisite for configuration control requirements in NIST 800-171, SOC 2, HIPAA, and PCI DSS. If you cannot demonstrate that two builds from the same source produce identical outputs, audit evidence for change management and system integrity controls is undermined. Use NixOS stable for production deployments. Support and warranty coverage under the [Terms of Service](legal/TERMS-OF-SERVICE.md) apply only to supported configurations (NixOS stable channel, documented system requirements).
