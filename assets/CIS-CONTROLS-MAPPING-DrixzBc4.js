...
| 2.3 | Address unauthorized software | **Private Nix Cache (Decision WVR-147)** locks host \`nix.settings.substituters\` to the private cache — unauthorized (unapproved) software cannot be installed because the derivation hash isn't in the signed allowlist. Deny-by-default at install time, not detect-after-the-fact | Planned (v2.3.0) | Weaver Team+ |
...
| 2.5 | Allowlist authorized software | **Exact match.** Approved Packages allowlist (Decision WVR-147) with manual approval at Solo/Team entry, automated approval pipeline at v3.1/v3.2 | Planned (v2.3.0) | Weaver Team+ |
| 2.6 | Allowlist authorized libraries | v3.1 SBOM extraction (Decision WVR-149 Shed Builder automated pipeline) identifies library dependencies; approval applies at derivation level | Planned (v3.1.0) | Fabrick |
| 2.7 | Allowlist authorized scripts | **Shed Builder Lane 4 (Decision WVR-149)** handles native Nix derivations — scripts installed via the cache are subject to the same approval flow | Planned (v2.3.0) | Weaver Team+ |
...
| 4.2 | Establish and maintain a secure configuration for network infrastructure | Managed bridges (Decision WVR-32); declarative firewall rules (v1.2); network topology in code | Implemented | Solo+ |
...
| 4.7 | Manage default accounts | First-run admin setup (no default credentials, Decision WVR-30); dedicated \`weaver\` service user with no interactive shell | Implemented | Free |
...
| Safeguard | Requirement | Weaver Implementation | Status | Tier |
|-----------|------------|----------------------|--------|------|
| 10.1 | Deploy and maintain anti-malware software | N/A at host level — Weaver doesn't manage endpoints. **However:** the v2.3 Private Nix Cache (Decision WVR-147) with signed approved-package allowlist provides **preventive** malware defense for software installed on managed hosts — unapproved/unsigned software cannot install. This is a stronger control than detective anti-malware (signature-based) because it's deny-by-default at install time | Partial (v1.0 infrastructure) · Preventive software control (v2.3 Decision WVR-147) | N/A (host AV) · Weaver Team+ (v2.3 prevention) |
...
| Safeguard | Requirement | Weaver Implementation | Status | Tier |
|-----------|------------|----------------------|--------|------|
| 11.1 | Establish and maintain a data recovery process | **Decision WVR-148 two-layer backup.** v1.0: declarative NixOS config = infrastructure state reproducible from git. v2.5: encrypted cloud-native backup (6 adapters: local FS, NFS, restic, borg at Solo; S3/Azure/GCS at Team) with retention, file-level restore, integrity verification. v2.7: Fabrick fleet-scale multi-target + cross-site replication + automated test restores | Implemented (infrastructure v1.0) · Planned data layer (v2.5) | Free (v1.0 config) · Weaver Team+ (v2.5 data) |
...
| 12.2 | Establish and maintain a secure network architecture | **Managed bridges (Decision WVR-32, WVR-114)** provide declarative network segmentation; IP pools; zone-aware topology map (Strands at v1.x, Loom at v3.0+) | Implemented | Solo+ |
...
| 16.4 | Establish and manage an inventory of third-party software components | \`package.json\` + lock files in git; Decision WVR-147 + Shed Builder v2.3 provides the *customer's* software-component inventory at install time | Implemented (Weaver deps) · Planned customer inventory (v2.3.0) | Free (Weaver) · Weaver Team+ (customer) |
| 16.5 | Use up-to-date and trusted third-party software components | Dependabot alerts; SHA-pinned GitHub Actions; lock file review in SECURITY-AUDIT.md. For customer-installed software: Private Nix Cache ensures only approved packages can install (Decision WVR-147) | Implemented (Weaver) · Planned customer side (v2.3.0) | Free / Weaver Team+ |
